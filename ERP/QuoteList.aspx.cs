...
{
	public partial class QuoteList : G
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			{
				InitNewQuoteList();
				NewQuote.Text=Root.rm.GetString("Quotxt2");

				if(!Page.IsPostBack)
				{
                    if(UC.AccessLevel==1)
                        FillRepeaterList(string.Format("SELECT * FROM QUOTES WHERE (CROSSTYPE>=0 AND OWNERID={0}) ",UC.UserId));
                    else
					    FillRepeaterList(string.Format("SELECT * FROM QUOTES WHERE (CROSSTYPE>=0 AND ({0})) ",GroupsSecure("QUOTES.GROUPS")));
                    QuoteEditing.FillQuoteStage(this.SearchQuoteStage, true, false);
		...
		{
			this.NewQuote.Click += new EventHandler(this.NewQuote_Click);
		...
		private void QuoteListRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			switch (e.Item.ItemType)
			{
				...
				case ListItemType.AlternatingItem:
                    ...
                    Literal lblPrint = (Literal)e.Item.FindControl("lblPrint");
                    ...
                    lblPrint.Text = "<img src=/i/printer.gif border=0 style=\"cursor:pointer\" onclick=\"CreateBox('/erp/printquote.aspx?render=no&qid=" + QuoteID.Text + "',event,600,300);\">";
		...
		private void btnSearch_Click(object sender, EventArgs e)
		{
            ...
            if(UC.AccessLevel==1)
                sbquery.AppendFormat("SELECT * FROM QUOTES WHERE (CROSSTYPE>=0 AND OWNERID={0}) ", UC.UserId);
            else
			    sbquery.AppendFormat("SELECT * FROM QUOTES WHERE (CROSSTYPE>=0 AND ({0})) ",GroupsSecure("QUOTES.GROUPS"));
            ...
            if (TextBoxSearchQuoteExpire.Text.Length>0)
			{
				...
				sbquery.AppendFormat("AND (DATEADD(N,{0},QUOTES.EXPIRATIONDATE){2} '{1} 00:00') ", mindiffstart.TotalMinutes.ToString(), QuoteExpire, Qtype);
