...
{
	public partial class DoubleBar_Chart : Page
	{
		protected void Page_Load(object sender, EventArgs e)
		{
			string[] s = new string[3] {"No data", "0", "0"};
			...
			wcEng.GridLines = GridLines.Vertical;
			...
			chart2.Line.Width = 2;
			chart2.Fill.Color = Color.FromArgb(90, Color.Beige);
			chart2.DataLabels.Visible = true;
			chart2.DataLabels.ShowValue = true;
			...
			for (int i = 0; i < s.Length; i++)
			{
				...
				cp.XValue = cp2.XValue = s[i];
				cp.YValue = float.Parse(s[i + 1]);
				cp2.YValue = float.Parse(s[i + 2]);
				chart.Data.Add(cp);
				chart2.Data.Add(cp2);
				i += 2;
			}
