...
{
	public delegate void SaveCompany();
	public delegate void HideControl();
	public partial class CRM_OppCompany : UserControl
	{
		public event SaveCompany Savecomp;
		public event HideControl Hidecontrol;
		...
		public void CompanyCrossCompetitorCommand(object source, RepeaterCommandEventArgs e)
		{
			switch (e.CommandName)
			{
				case "ModRelation":
					...
					break;
				case "SaveRelation":
					string idRel = DatabaseConnection.FilterInjection(((TextBox) e.Item.FindControl("TbxRelation")).Text);
					int idCross = int.Parse(((Literal) e.Item.FindControl("IDcross")).Text);
					DatabaseConnection.DoCommand(String.Format("UPDATE CRM_CROSSCONTACTCOMPETITOR SET RELATION='{0}' WHERE ID={1}", idRel, idCross));
		...
		{
			G.SetGoBack("/CRM/CRM_Opportunity.aspx?m=25&si=37&gb=1", new string[] {opId.ToString(), "ac",this.coCrossId.ToString(),this.coId.ToString()});
		...
		public void Tab_Click(string tabId)
		{
			switch (tabId)
			{
				...
				case "Table_Tab2":
					...
					break;
				case "Table_Tab3":
					Tabber.Selected = Table_Tab3.ID;
					FillRepeaterContactCompetitor();
					break;
				case "Table_Tab4":
					Tabber.Selected = Table_Tab4.ID;
					FillRepeaterActivityAzOp();
					break;
				case "Table_Tab6":
					Tabber.Selected = Table_Tab6.ID;
					Tabber.Selected=Table_Tab6.ID;
		...
		private void btnAddProduct_Click(object sender, EventArgs e)
		{
			...
			newprod.Qta = Convert.ToDouble(StaticFunctions.FixDecimal(EstQta.Text));

			decimal chFrom = 1;
			newprod.UnitPrice = Math.Round(StaticFunctions.FixDecimal(EstUp.Text)*chFrom, 2);
			newprod.Vat = (EstVat.Text.Length > 0) ? StaticFunctions.FixDecimal(EstVat.Text) : 0;
