...
{
	public partial class OppLoadFromCompany : G
	{
		...
		private void srcLeadbtn_Click(object sender, EventArgs e)
		{
			...
			Repeater1.DataSource = srccomp.GetCompanyTable();
			Repeater1.DataBind();
				...
				case "InsertOpp":
					foreach (RepeaterItem ri in Repeater1.Items)
					{
						...
						if (ck.Checked)
						{
							...
							InsertNewOpportunity(int.Parse(OppID.Text), int.Parse(ID.Text), "0");
						}
					}
					Response.Redirect("/crm/crm_opportunity.aspx?o=" + OppID.Text + "&tab=0");
					break;
				case "ViewSearch":
