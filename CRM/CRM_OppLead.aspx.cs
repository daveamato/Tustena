...
{
	public class CRM_OppLead : UserControl
	{
		protected HtmlTable NewCompany;
		protected HtmlContainerControl Table_Tab1;
		protected HtmlContainerControl Table_Tab3;
		protected HtmlContainerControl Table_Tab4;
		protected HtmlContainerControl Table_Tab6;

		protected HtmlTableCell tdTab1;
		protected HtmlTableCell tdTab3;
		protected HtmlTableCell tdTab4;
		protected HtmlTableCell tdTab6;

		protected LinkButton Tab1;
		protected LinkButton Tab3;
		protected LinkButton Tab4;
		protected LinkButton Tab6;
		...
		public void Page_Load(object sender, EventArgs e)
		{
			if (!Login())
			{
				Page.RegisterStartupScript("redirect","<script>parent.location='/login.aspx';</script>");
			}
			else
			{
				DeleteGoBack();
				...
				if(!Page.IsPostBack)
				{
					...
					coCrossId = int.Parse((Request.Params["CoCrossID"].Length>0)?Request.Params["CoCrossID"]:"0");
					ViewState["OpID"] = opId;
					ViewState["CoID"] = coId;
				...
				ViewForm.EnableViewState = false;
				if (!Page.IsPostBack)
				{
		...
		private void LoadData()
		{
			...
			Tab3.Text =Root.rm.GetString("CRMopptxt41");
			Tab4.Text =Root.rm.GetString("CRMopptxt32");
			Tab6.Text =Root.rm.GetString("CRMopptxt57");
			...
			Tab1.Text = DatabaseConnection.SqlScalar("SELECT TITLE FROM CRM_OPPORTUNITY WHERE ID=" + opId) + ":<br>" + CompanyNewCompany.Text;
			Tab1.Enabled = true;
			...
			tdTab1.Attributes.Add("Class", "TabHorizontal_HeaderSelected");
			tdTab6.Attributes.Add("Class", "TabHorizontal_Header");
			tdTab3.Attributes.Add("Class", "TabHorizontal_Header");
			tdTab4.Attributes.Add("Class", "TabHorizontal_Header");
...
DataTable dt;
 dt = DatabaseConnection.CreateDataset("SELECT ID,DESCRIPTION FROM CRM_OPPLOSTREASONS").Tables[0];
		...
		{
			SetGoBack("/CRM/CRM_Opportunity.aspx?m=25&si=37&gb=1",new string[]{opId.ToString(),"a"});
		...
		public void Tab_Click(Object sender, EventArgs e)
		{
			switch (((LinkButton) sender).ID)
			{
				...
				case "Tab3":
					...
					tdTab1.Attributes.Add("Class", "TabHorizontal_Header");
					tdTab3.Attributes.Add("Class", "TabHorizontal_HeaderSelected");
					tdTab4.Attributes.Add("Class", "TabHorizontal_Header");
				...
				case "Tab4":
					...
					tdTab1.Attributes.Add("Class", "TabHorizontal_Header");
					tdTab3.Attributes.Add("Class", "TabHorizontal_Header");
					tdTab4.Attributes.Add("Class", "TabHorizontal_HeaderSelected");
					tdTab6.Attributes.Add("Class", "TabHorizontal_Header");
				...
				case "Tab6":
					...
					tdTab3.Attributes.Add("Class", "TabHorizontal_Header");
					tdTab4.Attributes.Add("Class", "TabHorizontal_Header");
					tdTab6.Attributes.Add("Class", "TabHorizontal_HeaderSelected");
		...
		private void InsertNewAZ(int id)
		{
			...
			using (DigiDapter dg = new DigiDapter(sqlString.ToString()))
			{
				...
				if (id != -1 || (!dg.HasRows && id == -1))
				{
					...
					Page.RegisterStartupScript("refresh", "<script>clickElement(parent.document.getElementById(\"RefreshRepeaterLead\"));</script>");
				}
				else
				{
		...
		private void PrepareForNewCompany()
		{
			...
			tdTab3.Visible = false;
			tdTab4.Visible = false;
			tdTab6.Visible = false;

			Tab1.Text =Root.rm.GetString("CRMopptxt56");
			Tab1.Enabled = false;
			Table_Tab1.Visible = true;
			Table_Tab3.Visible = false;
			Table_Tab4.Visible = false;
		...
		private void InitializeComponent()
		{
			...
			this.RepeaterEstProduct.ItemDataBound+=new RepeaterItemEventHandler(RepeaterEstProduct_ItemDataBound);			this.Tab1.Click += new EventHandler(this.Tab_Click);
			this.Tab6.Click += new EventHandler(this.Tab_Click);
			this.Tab3.Click += new EventHandler(this.Tab_Click);
			this.Tab4.Click += new EventHandler(this.Tab_Click);
