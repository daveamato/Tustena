...
namespace Digita.Tustena.Admin
{
	public partial class groups : UserControl
	{
		...
		private void FillTable(int id)
		{
			...
			ListDip.DataSource = DatabaseConnection.CreateDataset(String.Format("SELECT * FROM GROUPS WHERE {0}", query));
			ListDip.DataBind();
		}
		...
		public void CategoryGridItemCommand(object source, DataGridCommandEventArgs e)
		{
			...
			{
				case "Edit":
					FillTable(Convert.ToInt32(((Literal) e.Item.FindControl("IDCat")).Text));
					FillGrid();
					FillListGroups();
					Groups_Grid.Visible = false;
					NewBtn.Visible = false;
					...
					break;
				case "Delete":
					delStr = "DELETE FROM GROUPS WHERE ID =" + int.Parse(((Literal) e.Item.FindControl("IDCat")).Text);
					DatabaseConnection.DoCommand(delStr);
		...
		public void Groups_Grid_ItemDataBound(Object sender, DataGridItemEventArgs e)
		{
			switch (e.Item.ItemType)
			{
				...
				case ListItemType.AlternatingItem:
					...
					DataSet ds;
					ds = DatabaseConnection.CreateDataset("SELECT * FROM GROUPS WHERE ID=" + int.Parse(id));
					DataRow dr = ds.Tables[0].Rows[0];
					string[] arryD = dr["Dependency"].ToString().Split('|');
					dr.Delete();
					ds.Clear();
